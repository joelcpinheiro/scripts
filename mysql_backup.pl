...
$site_name                   = 'Banco de Dados Zabbix';
$subject                     = "Backup concluido com sucesso,  $site_name";
...
$ftp_user                    = 'devadm';
$ftp_password                = 'Vy3f89seLND9pB90';

$ftp_dir                     = '/home/devadm/public_html/bkpzabbix';
                             # NOTE!!!!
...
if ( $backup_type eq 'mysqldump' )
        ...
        if ( $password_location eq 'cnf' )
            {
            system("$mysqldump_cmd", "--defaults-extra-file=$cnf_file", "--host=$db_host", "--quick", "--add-drop-table", "-c", "-l", "--result-file=$full_file", "$db_main", "$table_name");
            }
        else
            {
...
v.3.3 - Sep. 14, 2008    - Added code to make sure remote directory to delete ftp files
